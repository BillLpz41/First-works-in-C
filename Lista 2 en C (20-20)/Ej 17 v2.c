...
void base10baseM(int b10, char numM[], int baseM)
{
	...
	for(i=0; b10>0; i++)
	{
		aux= b10 % baseM;//aux almacena el residuo de la división
		b10 /= baseM;//divide el número en base 10 entre la base M
		
		if(aux>=0 && aux<10)//comprueba que aux esté en el rango de entre 0 y 9
			numM[i]= aux;
		else//si es mayor o igual a 10, se convierte el número a su forma de letra
			numM[i]= (aux + 'A') - 10;
	}	
...
void invertirCifras(char num[], int L)
{
	...
	num[L]='\0';//coloca el final de cadena en la última casilla
	
//	imprimir(final);//imprime el arreglo final
//	elDestructor(final);//libera la memoria del arreglo final
}
